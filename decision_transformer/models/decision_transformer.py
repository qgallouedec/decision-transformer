...
class DecisionTransformer(TrajectoryModel):
    ...
    def __init__(self, state_dim, act_dim, hidden_size, max_length=None, max_ep_len=4096, action_tanh=True, **kwargs):
        ...
        config = transformers.GPT2Config(
            vocab_size=1, n_embd=hidden_size, **kwargs  # doesn't matter -- we don't use the vocab
        )
    ...
    def get_action(self, states, actions, rewards, returns_to_go, timesteps, **kwargs):
        ...
        if self.max_length is not None:
            ...
            returns_to_go = torch.cat(
                [
                    torch.zeros(
                        (returns_to_go.shape[0], self.max_length - returns_to_go.shape[1], 1), device=returns_to_go.device
                    ),
                    returns_to_go,
                ],
                dim=1,
